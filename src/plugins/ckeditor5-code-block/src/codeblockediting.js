  ...
  init () {
    ...
    this.listenTo(model, 'getSelectedContent', (evt, [selection]) => {
      ...
      model.change(writer => {
        ...
        } else { // "f[oo]"                          ->   <$text code="true">oo</text>
 ...
 */
  afterInit () {
    ...
    this.listenTo(editor.editing.view.document, 'enter', (evt, data) => {
      ...
      leaveBlockStartOnEnter(editor, data.isSoft) || leaveBlockEndOnEnter(editor, data.isSoft) || breakLineOnEnter(editor)
...
function leaveBlockEndOnEnter (editor, isSoftEnter) {
  ...
  // <codeBlock>foo[<softBreak></softBreak>    ]</codeBlock>
  //
    nodeBefore.is('text') && !nodeBefore.data.match(/\S/) && nodeBefore.previousSibling && nodeBefore.previousSibling.is('softBreak')
  ) {
